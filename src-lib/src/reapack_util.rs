...
pub fn get_default_reapack_shared_lib_file(
    ...
    let file_name = get_os_specific_reapack_file_name()?;
    Some(
...
pub async fn get_correct_reapack_asset(release: Release) -> anyhow::Result<Asset> {
    let os_specific_file_name = get_os_specific_reapack_file_name()
        .context("Couldn't determine OS-specific ReaPack file name")?;
    release
        .assets
        .into_iter()
...
pub fn get_os_specific_reapack_file_name() -> Option<&'static str> {
    let name = match ReaperTarget::AT_BUILD_TIME? {
